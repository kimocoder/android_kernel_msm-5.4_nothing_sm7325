...
u32 rtw_start_drv_threads(_adapter *padapter)
{
	...
	if (is_primary_adapter(padapter)) {
		if (padapter->cmdThread == NULL) {
			RTW_INFO(FUNC_ADPT_FMT " start RTW_CMD_THREAD2\n", FUNC_ADPT_ARG(padapter));
			padapter->cmdThread = kthread_run(rtw_cmd_thread2, padapter, "RTW_CMD_THREAD2");
...
u8 rtw_free_drv_sw(_adapter *padapter)
{
	...
	rtw_free_evt_priv2(&padapter->evtpriv);
...
static int netdev_close(struct net_device *pnetdev)
{
	...
	{
		/* void nat25_db_cleanup2(_adapter *priv); */
...
int rtw_suspend_common(_adapter *padapter)
{
	...
	if (rtw_mi_check_status2(padapter, MI_AP_MODE) == _FALSE) {
#ifdef CONFIG_WOWLAN
		if (check_fwstate(pmlmepriv, _FW_LINKED))
	...
	} else if (rtw_mi_check_status2(padapter, MI_AP_MODE)) {
#ifdef CONFIG_AP_WOWLAN
...
int rtw_resume_common(_adapter *padapter)
{
	...
	if (rtw_mi_check_status2(padapter, WIFI_AP_STATE) == _FALSE) {
#ifdef CONFIG_WOWLAN
		if (pwrpriv->wowlan_mode == _TRUE)
	...
	} else if (rtw_mi_check_status2(padapter, WIFI_AP_STATE)) {
#ifdef CONFIG_AP_WOWLAN
