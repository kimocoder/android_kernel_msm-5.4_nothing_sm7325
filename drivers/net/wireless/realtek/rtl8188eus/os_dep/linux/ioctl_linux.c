			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	if (wrqu->freq.m <= 1000) {
		...
		/* If freq is invalid, rtw_freq2ch2() will return channel 1 */
		padapter->mlmeextpriv.cur_channel = rtw_freq2ch2(freq);
		RTW_INFO("%s: set to channel %d\n", __func__, padapter->mlmeextpriv.cur_channel);
	}
			  ...
			  char *extra)
{
	...
	*/
	...
	rtw_set_802_11_authentication_mode2(padapter, authmode);
	...
	case 0x7F:
		...
		case 0xdd: { /* registers dump , 0 for mac reg,1 for bb reg, 2 for rf reg */
			if (extra_arg == 0)
				...
				bb_reg_dump_ex2(RTW_DBGDUMP, padapter);
		}
			break;
...
static s32 createpseudoadhoc(PADAPTER padapter)
{
	...
	authmode = Ndis802_11AuthModeOpen;
	err = rtw_set_802_11_authentication_mode2(padapter, authmode);
	if (err == _FALSE)
		return _FAIL;
