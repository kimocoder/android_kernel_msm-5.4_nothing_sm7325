...
sint recvframe_chkmic(_adapter *adapter,  union recv_frame *precvframe)
{
	...
	if (prxattrib->encrypt == _TKIP_) {
		...
		if (stainfo != NULL) {
			...
			/* rtw_seccalctkipmic2(&stainfo->dot11tkiprxmickey.skey[0],pframe,payload, datalen ,&miccode[0],(unsigned char)prxattrib->priority); */ /* care the length of the data */

			rtw_seccalctkipmic2(mickey, pframe, payload, datalen , &miccode[0], (unsigned char)prxattrib->priority); /* care the length of the data */
...
union recv_frame *decryptor(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (prxattrib->encrypt && !prxattrib->bdecrypted) {
		...
		RTW_INFO(ADPT_FMT" - sec_type:%s DO SW decryption\n",
			ADPT_ARG(padapter), security_type_str2(prxattrib->encrypt));
#endif
		...
		case _AES_:
			...
			res = rtw_aes_decrypt2(padapter, (u8 *)precv_frame);
			break;
