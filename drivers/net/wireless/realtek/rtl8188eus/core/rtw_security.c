...
static const char *_security_type_str2[] = {
	"N/A",
	"WEP40",
	"TKIP",
	"TKIP_WM",
...
const char *security_type_str2(u8 value)
{
...
#endif
		return _security_type_str2[value];
	return NULL;
}
...
void rtw_secmicappend2(struct mic_data *pmicdata, u8 *src, u32 nbytes)
{
	/* This is simple */
	while (nbytes > 0) {
		rtw_secmicappendbyte2(pmicdata, *src++);
		nbytes--;
	}
}
...
void rtw_secgetmic(struct mic_data *pmicdata, u8 *dst)
{
	...
	rtw_secmicappendbyte2(pmicdata, 0x5a);
	rtw_secmicappendbyte2(pmicdata, 0);
	rtw_secmicappendbyte2(pmicdata, 0);
	rtw_secmicappendbyte2(pmicdata, 0);
	rtw_secmicappendbyte2(pmicdata, 0);
...
void rtw_seccalctkipmic2(u8 *key, u8 *header, u8 *data, u32 data_len, u8 *mic_code, u8 pri)
{
	...
	if (prxattrib->encrypt == _AES_) {
		...
		if (stainfo != NULL) {
			...
			if (IS_MCAST(prxattrib->ra)) {
				...
				/* RTW_INFO("rx bc/mc packets, to perform sw rtw_aes_decrypt2\n"); */
				/* prwskey = psecuritypriv->dot118021XGrpKey[psecuritypriv->dot118021XGrpKeyid].skey; */
...
u8 rtw_handle_tkip_countermeasure2(_adapter *adapter, const char *caller)
{
	struct security_priv *securitypriv = &(adapter->securitypriv);
	u8 status = _SUCCESS;
